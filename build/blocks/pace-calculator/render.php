...
if ( ! function_exists( 'rp_theme_format_cell_time' ) ) {
	function rp_theme_format_cell_time( $total_minutes ) {
		if ( $total_minutes <= 0 ) {
			return '--:--';
		}
...
<div <?php echo $wrapper_attributes; ?> data-wp-interactive="rp-multi-block" <?php echo wp_interactivity_data_wp_context( $initial_context ); ?>>
	<div class="rp-pace-inputs">
		<label>
			<span class="rp-pace-label"><?php esc_html_e( 'Pace', 'advanced-multi-block' ); ?></span>
			<input
				type="number"
				min="0"
				max="59"
		...
		<span class="rp-pace-unit">
			<?php esc_html_e( 'min/', 'advanced-multi-block' ); ?><span data-wp-text="state.unitLabel">km</span>
		</span>
		<button
			class="rp-unit-toggle"
			data-wp-on--click="actions.toggleUnit"
			data-wp-text="state.unitToggleLabel"
		><?php esc_html_e( 'Switch to mi', 'advanced-multi-block' ); ?></button>
	</div>

	<div class="rp-tables-wrapper" data-wp-watch="callbacks.renderRows">
		<div class="rp-section rp-section--key">
			<h3 class="rp-section-label"><?php esc_html_e( 'Key Distances', 'advanced-multi-block' ); ?></h3>
			<div class="rp-table-wrap">
				<table class="rp-pace-table">
					<thead>
						<tr>
							<th><?php esc_html_e( 'Distance', 'advanced-multi-block' ); ?></th>
						...
						<tr class="rp-row-race">
							...
							foreach ( $offsets as $offset ) :
								...
								if ( $cell_seconds <= 0 ) :
									...
									?>
									<td><?php echo rp_theme_format_cell_time( $total_minutes ); ?></td>
									<?php
								endif;
							endforeach;
							?>
		...
		>
			...
			<span class="rp-toggle-label" data-wp-text="state.toggleLabel"><?php esc_html_e( 'Show full 1K–50K breakdown', 'advanced-multi-block' ); ?></span>
		</button>
		...
		<div
			...
			hidden
		>
			<h3 class="rp-section-label"><?php esc_html_e( 'Full 1K–50K Breakdown', 'advanced-multi-block' ); ?></h3>
			<div class="rp-table-wrap">
				<table class="rp-pace-table">
					<thead>
						<tr>
